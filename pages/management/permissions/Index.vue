<template>
  <v-container fluid class="py-6 px-6">
    <div class="d-flex align-center justify-space-between mb-5">
      <div>
        <div class="text-h5 font-weight-bold">{{ $t('permissions.title') }}</div>
      </div>
    ...
    <v-card rounded="xl" elevation="0" border class="table-responsive-card">
      ...
      >
        <template #item.key="{ item }">
          <v-chip size="x-small" color="primary" variant="tonal" label>{{ item.key }}</v-chip>
        </template>
...
const {
  ...
  onCloseDelete,
  resetFilters,
} = useCrudPage<PermissionModel, typeof PermissionService>(PermissionService, {
  filters: { search: '' },
})
/* END DEFINE STATE */
</script>
